...
- `eval-source-map`： 每个模块使用`eval()`执行，并且source map转换为DataUrl后添加到eval()中。
...
上面的配置告诉`webpack-dev-server`，在`localhost:8080`下建立服务器，将dist目录下的文件作为可访问文件。此时文件更改不仅受到监听，同时还能触发浏览器的自动刷新。
